classdef Gain < synConst.synConst
    
    % SYNCONST.GAIN specifies a constraint on H-infinity performance
    %
    %   factor*||Wout*G(outputs,inputs)*Win|| <= bound
    %
    % Use:
    ...
    methods
        ...
        function obj = Gain(varargin)
            ...
            % SYNCONST.GAIN specifies a constraint on the H-infinity gain
            %
            %   factor*||Wout*G(outputs,inputs)*Win|| <= bound
            %
            % Use:
        ...
        function varargout = char(obj)
            ...
            % CHAR method for synConst.GainH2 class
            
            if isempty(obj.inputs) || isempty(obj.outputs)
                varargout{1} = sprintf('\tEmpty synConst.Gain constraint.\n');
                ...
                if isa(obj.bound,'sdpvar') || isinf(obj.bound)
                    str1 = 'min';
                    str2 = sprintf('%4.2f*||Tzw||_inf',obj.factor);
                    str3 = sprintf('z=%s, w=%s',strOuts,strIns);
                else
                    str1 = 's.t.';
                    str2 = sprintf('%4.2f*||Tzw||_inf < %4.3f',obj.factor,obj.bound);
                    str3 = sprintf('z=%s, w=%s',strOuts,strIns);
                end
