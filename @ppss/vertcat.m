...
% VERTCAT returns a ppss model corresponding to the vertical concatenation
%
% Use:
...
if (nargin == 2)
    ...
    if (ni1 ~= ni2)
        error('PGSS:VERCAT:inputError','G1 or G2 must have the same number of inputs')
    end
    ...
    if isa(G1,'ppss') && isa(G2,'ppss')
        ...
        if bool && (nv1 == nv2)
            ...
            Bo(ns1+ns2,ni1,nv1) = 0;
            Co(no1+no2,ns1+ns2,nv1) = 0;
            Do(no1+no2,ni1,nv1) = 0;
            for ii = 1:nsys(G1)
            ...
            error('PPSS:VERCAT:inputError','G1 or G2 must have the same parameter set and the same number of vertices')
        end
    ...
    elseif (isa(G1,'ss') || isa(G1,'zpk') || isa(G1,'tf') || isnumeric(G1))...
        ...
        Bo = zeros(ns1+ns2,ni1,nv);
        Co = zeros(no1+no2,ns1+ns2,nv);
        Do = zeros(no1+no2,ni1,nv);
    ...
    elseif (isa(G2,'ss') || isa(G2,'zpk') || isa(G2,'tf') || isnumeric(G2))...
        ...
        error('PPSS:VERCAT','G1 or G2 must be LTI systems or ppss objects')
    end
