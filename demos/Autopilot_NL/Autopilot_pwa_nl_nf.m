...
%
% Forcing bk and dk constant to avoid filtering the output channel
...
sb1 = sumblk('err = nc - n - dn1r');
sb2 = sumblk('y = q + dn2r');
sb3 = sumblk('up = ua + dr');
sb4 = sumblk('e = ref - n');
pdGaw = connect(pdG,sb1,sb2,sb3,sb4,Wref,We,Wdc,Wd,Wn1,Wn2,Act,...
                {'nc','d','dn1','dn2','dc'},...
                {'er','edc','err','y'});
...
ctrlfcn.pdIn = 0;
[pdKc,constOut] = lpvsyn(pdGaw,3:4,5,const,[],ctrlfcn,opt);
gCte = constOut(1).bound;

% different options for PWA Lyapunov function
[pdKpwa,constOut] = lpvsyn(pdGaw,3:4,5,const,'pwa',ctrlfcn,opt);
gpwa = constOut(1).bound;

[pdKpwax,constOut] = lpvsyn(pdGaw,3:4,5,const,'pwaX',ctrlfcn,opt);
gpwax = constOut(1).bound;

[pdKpway,constOut] = lpvsyn(pdGaw,3:4,5,const,'pwaY',ctrlfcn,opt);
gpway = constOut(1).bound;

[pdKpwadx,constOut] = lpvsyn(pdGaw,3:4,5,const,'pwadX',ctrlfcn,opt);
gpwadx = constOut(1).bound;

[pdKpwady,constOut] = lpvsyn(pdGaw,3:4,5,const,'pwadY',ctrlfcn,opt);
...
for ii = 1:size(gridSet,2)
    ...
    % H-infinite design
    Gaw = connect(G,sb1,sb2,sb3,sb4,Wref,We,Wdc,Wd,Wn1,Wn2,Act,...
                {'nc','d','dn1','dn2','dc'},...
                {'er','edc','err','y'});
