...
%
% Normalized Version to improve numerical issues
...
D = cat(3,d0,d1,d2);
% parameter set
Pset = pset.Box([Zmin Zmax; Mmin Mmax],[-10 10]*0,{'Z','M'});

% affine lpv model
pdG = pass(A,B,C,D,Pset,'InputName','u','OutputName','y');

% normalized LPV model wrt to the parameters
pdGn = normalize(pdG);
...
% augmented plant
er  = sumblk('e = r - y(1)');
pdGau = connect(pdGn,er,{'r','u'},{'e','u','e','y(2)'});
% augmented plant + weight
...
Pset_gr = pset.Grid(pdGaw.parset.range,5,pdGaw.parset.rate);
pdGaw_gr = pdGaw;
...
[pdK5,constOut]  = lpvsyn(pdGaw,{'e','y(2)'},'u',const,lyapSet);
