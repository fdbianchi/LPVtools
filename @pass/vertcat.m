...
% VERTCAT returns a pass model corresponding to the vertical concatenation
%
% Use:
...
if (nargin == 2)
    ...
    if (ni1 ~= ni2)
        error('PASS:vertcat:inputError','G1 or G2 must have the same number of inputs')
    end
    ...
    if isa(G1,'pass') && isa(G2,'pass')
        ...
        if bool
            ...
            error('PASS:vertcat:inputError','G1 or G2 must have the same parameter set')
        end
    ...
    elseif (isa(G2,'ss') || isa(G2,'zpk') || isa(G2,'tf')) && isa(G1,'pass')
        ...
        error('PASS:vertcat:inputError','G1 and G2 must be LTI models or pass objects')
