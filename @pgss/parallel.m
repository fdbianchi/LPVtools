...
%
% where:
%   G1, G2 are pgss objects or ss, tf, or zpk objects
...
if (nargin == 2)
    ...
    % only models in the inputs arguments.
    ...
    if (no1 ~= no2) || (ni1 ~= ni2)
        error('PGSS:parallel:inputError','All models must have the same number of inputs and outputs')

    else
        
        if isa(G1,'pgss') && isa(G2,'pgss')
            % case both models lpv
            ...
            if (bool1 && bool2)
                ...
                error('PGSS:parallel:inputError','G1 or G2 must have the same parameter set and functions')
            end
        ...
        elseif (isa(G2,'ss') || isa(G2,'zpk') || isa(G2,'tf')) && isa(G1,'pgss')
            ...
            error('PGSS:Parallel:inputError','G1 or G2 must be a LTI models or pgss objects')
...
elseif (nargin == 6)
    ...
    if (li1 ~= li2)
        error('PGSS:Parallel:inputError','IN1 and IN2 must have the same length');
    elseif (lo1 ~= lo2)
        error('PGSS:Parallel:inputError','OUT1 and OUT2 must have the same length');
    elseif any(inp1 <= 0) || any(inp1 > ni1)
        error('PGSS:Parallel:inputError','IN1 must be valid model inputs');
    elseif any(inp2 <= 0) || any(inp2 > ni2)
        error('PGSS:Parallel:inputError','IN2 must be valid model inputs');
    elseif any(out1 <= 0) || any(out1 > no1)
        error('PGSS:Parallel:inputError','OUT1 must be valid model outputs');
    elseif any(out2 <= 0) || any(out2 > no2)
        error('PGSS:Parallel:inputError','OUT2 must be valid model outputs');
    end
    ...
    error('PGSS:Parallel:inputError','The number of input arguments must be 2 or 6');
end
