...
%   pdK is the controller (pgss, pcss or LTI object)
%
% when pdK is a pgss and the meas/ctrl channel is not parameter dependent, 
% the resulting model is also a pgss object. The meas or ctrl channel can be 
...
if isa(pdK,'pgss') && (nargin < 3)
    ...
    if isequal(pdG.parset, pdK.parset) && isequal(pdG.parfcn, pdK.parfcn)
        ...
        if any(d22,'all')
            error('PGSS:LFT','d22 must be zero')
        end
       ...
       pdGout = lft(pdG,pdK,pdG.parset.points); 
       warning('PGSS:LFT',...
           ['The parameter set and functions of both models must be equal\n',...
            'therefore, the LFT is evaluated at a set of points.'])
...
elseif (isa(pdK,'ss') || isa(pdK,'zpk') || isa(pdK,'tf')) && (nargin < 3)
    ...
    if all(ioDiag(2:3)) && any(dk)
        error('PGSS:LFT','the resulting model will not be affine\nbecause %s',ioMsg)
    end
...
elseif isa(pdK,'pcss') || (nargin == 3)
    ...
    for ii = 1:nv
        ...
        [a,b1,b2,c1,c2,d11,d12,d21,d22] = parsysdata(G(:,:,ii),[ny nu]);
        if any(d22,'all')
            error('PGSS:LFT','d22 must be zero')
        end
