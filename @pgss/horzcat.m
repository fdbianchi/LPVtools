...
% HORZCAT returns a pgss model corresponding to the horizontal concatenation
%
% Use:
...
if (nargin == 2)
    ...
    if isa(G1,'pgss') && isa(G2,'pgss')
        ...
        if (bool1 && bool2)
            ...
            error('PGSS:horzcat:inputError','G1 or G2 must have the same parameter set and functions')
        end
    ...
    elseif (isa(G2,'ss') || isa(G2,'zpk') || isa(G2,'tf')) && isa(G1,'pgss')
        ...
        error('PGSS:horzcat:inputError','G1 and G2 must be LTI models or pgss objects')
