...
% parameter set
setP = pset.Box([Zmin Zmax ; Mmin Mmax]);
...
% this syntax produce a general model        
fcn = @(p) p;
pdG1a = pgss(a,b,c,d,setP,fcn);     
% this syntax produce an affine model
pdG1  = pgss(a,b,c,d,setP,fcn,...
...
%
% sintaxis 1
pdG2 = pgss(sys,[Zmin Zmax; Mmin Mmax],fcn);
pdG2.u = 'u';
pdG2.y = {'y1','y2'};

% sintaxis 2
pdG3 = pgss(sys,setP,fcn);
pdG3.u = 'u';
pdG3.y = {'y1','y2'};

% sintaxis 3: from uss object
...
pdG1s2 = pdG1('y(2)','u');
if ~isequal(a,pdG1s1.A) || ~isequal(b(:,idx2,:),pdG1s1.B) ||...
        ~isequal(c(idx1,:,:),pdG1s1.C) || ~isequal(d(idx1,idx2,:),pdG1s1.D)
    error('Error in SUBSREf with 2 argument')
elseif ~isequal(a,pdG1s2.A) || ~isequal(b(:,idx2,:),pdG1s2.B) || ...
        ~isequal(c(idx1,:,:),pdG1s2.C) || ~isequal(d(idx1,idx2,:),pdG1s2.D)
    error('Error in SUBSREf with 2 argument')
else
...
% to uss:
sys2 = uss(pdG1);
pval = pdG1.parfcn(pdG1.parset.points);
sys1e = usubs(sys2,'p1',pval(1,:),'p2',pval(2,:));
...
pdG1d = denormalize(pdG1n, pdG1.parfcn);
figure
